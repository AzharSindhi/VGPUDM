...
class ViPCDataLoader(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        result['class_index'] = views.numpy().astype(np.float32)
...
if __name__ == "__main__":
    ...
    # complets = dataset['completes']
    
    # np.save(os.path.join(path, f"ViPC_{status}_{category}_R{R}_all_names.npy"), names)
    # np.save(os.path.join(path, f"ViPC_{status}_{category}_R{R}_all_images.npy"), images)
    # np.save(os.path.join(path, f"ViPC_{status}_{category}_R{R}_all_partials.npy"), partials)
    # np.save(os.path.join(path, f"ViPC_{status}_{category}_R{R}_all_completes.npy"), complets)

    # Calculate size in GB
    # test_images = torch.from_numpy(dataset['images'])
